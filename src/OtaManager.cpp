...
void OtaManager::_checkUpdateTaskRunner(void* pvParameters) {
    ...
    esp_err_t wdt_init_result = esp_task_wdt_init(&twdt_config_check);
    if (wdt_init_result != ESP_OK && wdt_init_result != ESP_ERR_INVALID_STATE) {
        Serial.printf("WDT init error: %s\n", esp_err_to_name(wdt_init_result));
    }

    esp_err_t wdt_add_result = esp_task_wdt_add(NULL);
    if (wdt_add_result != ESP_OK) {
        Serial.printf("WDT add task error: %s\n", esp_err_to_name(wdt_add_result));
    }
